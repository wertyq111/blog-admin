        ...
        <el-col :lg="6" style="margin-bottom: 15px;">
          ...
          <ele-pro-table
            ...
            <template slot="toolbar">
              <el-button
                v-if="permission.includes('sys:custom-dictionary:add')"
                class="ele-btn-icon"
                icon="el-icon-plus"
                size="small"
                type="primary"
              ...
              <el-button
                v-if="permission.includes('sys:custom-dictionary:edit')"
                :disabled="!current"
                class="ele-btn-icon"
                icon="el-icon-edit"
                size="small"
              ...
              <el-button
                v-if="permission.includes('sys:custom-dictionary:delete')"
                :disabled="!current"
                class="ele-btn-icon"
                icon="el-icon-delete"
                size="small"
...
export default {
  ...
  data() {
    return {
      ...
      // 表格搜索条件
      where: {
        deptId: this.$store.state.user ? this.$store.state.user.dept_id : 0
      },
      // 当前编辑数据
      current: null,
      // 是否显示编辑弹窗
  ...
  methods: {
    ...
    remove() {
      this.$confirm('确定要删除选中的字典吗?', '提示', {
        ...
        this.$http.post('/customdict/delete?id=' + this.current.id).then(res => {
          loading.close();
          if (res.data.code === 0) {
