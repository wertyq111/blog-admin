...
<template>
  <div>
    <el-dialog
      ...
      :lock-scroll="false"
      :title="(isUpdate?'修改':'添加') + (form.is_return===0?'采购开票单':'采购开票退货单')"
      :visible="visible"
      ...
      <el-form
        ...
        <el-row :gutter="15">
          <el-col :span="8">
            <el-form-item label="订单状态:" prop="status">
              <el-tag type="info" effect="dark" v-if="form.status == business.invoiceStatus.wait">待提交</el-tag>
              <el-tag effect="dark" v-if="form.status == business.invoiceStatus.open">已开立</el-tag>
              <el-tag type="success" effect="dark" v-if="form.status == business.invoiceStatus.audited">已审核</el-tag>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="发票类型:" prop="invoice_type_id">
              <el-select
                ...
                filterable
                placeholder="-请选择发票类型-"
                :disabled="(form.status == business.invoiceStatus.open
                          || form.status == business.invoiceStatus.audited)"
                size="small">
        ...
        <el-row :gutter="15">
          ...
          <el-col :span="8">
            <el-form-item label="代垫单位:" prop="advance_unit_id">
              <el-select
                v-model="form.advance_unit_id"
                class="ele-block"
                clearable
                filterable
                placeholder="-请选择代垫单位-"
                ...
                <el-form-item :prop="'order_items.' + scope.$index + '.' + customItem.code" :rules="rules[customItem.code]">
                  <el-select
                    ...
                    :disabled="(customItem.status == business.customStatus.close
                          || form.status == business.invoiceStatus.open
                          || form.status == business.invoiceStatus.audited)"
                    size="small">
                    <el-option
              ...
              <template slot-scope="scope">
                <el-form-item :prop="'order_items.' + scope.$index + '.order_number'" :rules="rules.order_number">
                  <el-input
                    ...
                    clearable
                    :disabled="(
                    form.status == business.invoiceStatus.wait
                    || form.status == business.invoiceStatus.open
                    || form.status == business.invoiceStatus.audited)"
      ...
      <div slot="footer">
        <el-button
          v-if="permission.includes('sys:invoice:cancelOpen')"
          :disabled="!(isUpdate && (form.status == business.invoiceStatus.open))"
          @click="cancelOpen(form.id)"
        >
          修改
        </el-button>
        <el-button
          v-if="permission.includes('sys:invoice:audit')"
          :disabled="!(isUpdate && (form.status == business.invoiceStatus.open))"
          @click="audit(form.id)"
        >
          审核
        ...
        <el-button
          v-if="permission.includes('sys:invoice:restoreAudit')"
          :disabled="!(isUpdate && form.status == business.invoiceStatus.audited)"
...
import {
  changeAmount, changeAmountWithTax,
  changePrice,
  changeQuantity, changeTax,
  changeTaxRate,
  changeUnitPrice,
  getAmount,
  getAmountWithTax,
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        advance_unit_id: [
          {required: false, message: '请选择代垫单位', trigger: 'blur'}
        ],
        invoice_number: [
  ...
  watch: {
    data() {
      if (this.data) {
        this.fillFormByRemote(this.data.invoice_id);
        this.isUpdate = true;
      } else {
    ...
    async referenceData() {
      if (this.referenceData) {
        // 把要拿的信息重新定义
        let fillData = {
          status: business.invoiceStatus.wait,
        ...
        for (let i = 0; i < oldOrderItems.length; i++) {
          let arrival_order_item_id = oldOrderItems[i].arrival_order_item_id;
          let params = {id: arrival_order_item_id};
          ...
          await this.$http.get("/arrivalorder/itemInfo", {params: params}).then(res => {
            if (res.data.code === 0) {
              let arrivalOrderData = res.data.data;
              unit_price = arrivalOrderData.unit_price;
              tax_rate = arrivalOrderData.tax_rate;
              quantity = oldOrderItems[i].remain_invoice_quantity;
              orderItems.push(Object.assign({
                warehouse_entry_order_item_id: oldOrderItems[i].id,
                ...
                tax_rate: tax_rate,
                brand:oldOrderItems[i].brand,
                specification:oldOrderItems[i].specification,
                texture:oldOrderItems[i].texture,
  ...
  methods: {
    ...
    fillFormByRemote(order_id) {
      ...
      this.$http.get("/invoice/info", {params: params}).then(res => {
        if (res.data.code === 0) {
    ...
    addOrderItem() {
      ...
      if (this.form.status == business.invoiceStatus.changing) {
    ...
    cancelOpen(id) {
      ...
      this.$http.post('/invoice/cancelOpen', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
    ...
    audit(id) {
      const loading = this.$loading({lock: true});
      this.$http.post('/invoice/audit', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.fillFormByRemote(id);
    ...
    restoreAudit(id) {
      const loading = this.$loading({lock: true});
      this.$http.post('/invoice/restoreAudit', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.fillFormByRemote(id);
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.order_items && this.form.order_items.length > 0) {
            this.loading = true;
            this.$http.post('/invoice/edit', this.form).then(res => {
              this.loading = false;
              if (res.data.code === 0) {
