...
export default {
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/inventoryarchive/edit', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 0) {
    ...
    initFormData(data) {
      // 初始化默认值
      let form = {status: 1, customs_rate: "1.0000000", sale_rate: "13.00", purchase_rate: "13.00"};
      if (data) {
