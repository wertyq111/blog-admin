...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        username: [
          {
            ...
            validator: (rule, value, callback) => {
              ...
              this.$http.get('/companyuser/checkUser?username=' + value).then(res => {
                if (!this.isUpdate && res.data.code !== 0) {
        ...
        password: [
          {required: false, pattern: /^[\S]{5,18}$/, message: '密码必须为5-18位非空白字符', trigger: 'blur'}
        ],
        mobile: [
  ...
  methods: {
    ...
    save() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          this.$http.post('/companyuser/edit', this.form).then(res => {
            this.loading = false;
            if (res.data.code === 0) {
    ...
    /* 查询角色列表 */
    queryRoles() {
      this.$http.get('/role/getCompanyList').then(res => {
        if (res.data.code === 0) {
