...
<template>
  <div>
    <el-dialog
      ...
      :lock-scroll="false"
      :title="(isUpdate?'修改':'添加') + (form.is_return===0?'发货订单':'退货订单')"
      :visible="visible"
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="订单状态:" prop="status">
              <el-tag type="info" effect="dark" v-if="form.status == business.shipmentOrderStatus.wait">待提交</el-tag>
              <el-tag effect="dark" v-if="form.status == business.shipmentOrderStatus.open">已开立</el-tag>
              <el-tag type="success" effect="dark" v-if="form.status == business.shipmentOrderStatus.audited">已审核</el-tag>
              <el-tag type="info" effect="dark" v-if="form.status == business.shipmentOrderStatus.changing">变更中</el-tag>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="15">
          <el-col :span="8">
            <el-form-item label="业务类型:" prop="business_type_id">
              <el-select
                ...
                filterable
                placeholder="-请选择业务类型-"
                :disabled="(form.status == business.shipmentOrderStatus.open
                          || form.status == business.shipmentOrderStatus.audited
                          || form.status == business.shipmentOrderStatus.changing)"
                size="small">
          ...
          <el-col :span="8">
            <el-form-item label="单据号:" prop="shipment_order_number">
              <el-input
                v-model="form.shipment_order_number"
                :disabled=true
                :maxlength="255"
                clearable
              ...
              <template slot-scope="scope">
                <el-form-item :prop="'order_items.' + scope.$index + '.warehouse_id'" :rules="rules.warehouse_id">
                  <el-select
                    ...
                    :disabled="(form.status == business.shipmentOrderStatus.open
                          || form.status == business.shipmentOrderStatus.audited
                          || (form.status == business.shipmentOrderStatus.changing && scope.row.canChange !== true))"
                  >
                ...
                <el-form-item :prop="'order_items.' + scope.$index + '.' + customItem.code" :rules="rules[customItem.code]">
                  <el-select
                    ...
                    :disabled="(customItem.status == business.customStatus.close
                          || form.status == business.shipmentOrderStatus.open
                          || form.status == business.shipmentOrderStatus.audited
                          || form.status == business.shipmentOrderStatus.changing)"
                    size="small">
                    <el-option
              ...
              <template slot-scope="scope">
                <el-form-item :prop="'order_items.' + scope.$index + '.order_number'" :rules="rules.order_number">
                  <el-input
                    ...
                    clearable
                    :disabled="(
                    form.status == business.shipmentOrderStatus.wait
                    || form.status == business.shipmentOrderStatus.open
                    || form.status == business.shipmentOrderStatus.audited
                    || form.status == business.shipmentOrderStatus.changing)"
      ...
      <div slot="footer">
        <el-button
          v-if="permission.includes('sys:shipmentorder:cancelOpen')"
          :disabled="!(isUpdate && (form.status == business.shipmentOrderStatus.open))"
          @click="cancelOpen(form.id)"
        >
          修改
        </el-button>
        <el-button
          v-if="permission.includes('sys:warehouseoutorder:edit') && form.is_return === 0"
          :disabled="!(isUpdate && form.status == business.shipmentOrderStatus.audited)"
          @click="openWarehouseOutEdit(form)"
        >
          出库
        </el-button>
        <el-button
          v-if="permission.includes('sys:warehouseoutorder:edit') && form.is_return === 1"
          :disabled="!(isUpdate && form.status == business.shipmentOrderStatus.audited)"
          @click="openWarehouseOutEdit(form, 1)"
        >
          出库退货
        </el-button>
        <el-button
          v-if="permission.includes('sys:shipmentorder:audit')"
          :disabled="!(isUpdate && (form.status == business.shipmentOrderStatus.open))"
          @click="audit(form.id)"
        >
          审核
        ...
        <el-button
          v-if="permission.includes('sys:shipmentorder:restoreAudit')"
          :disabled="!(isUpdate && form.status == business.shipmentOrderStatus.audited)"
...
<!--        <el-button-->
<!--          v-if="permission.includes('sys:shipmentorder:change')"-->
<!--          :disabled="!(isUpdate && (form.status == business.shipmentOrderStatus.audited))"-->
...
import {
  ...
  changeAmountWithTax,
  changeDiscount,
  changeDiscountRate, changeDiscountRate2
...
export default {
  ...
  components: {Treeselect, WarehouseOutOrderEdit},
  data() {
    return {
  ...
  watch: {
    data() {
      if (this.data) {
        this.fillFormByRemote(this.data.shipment_order_id);
        this.isUpdate = true;
      } else {
    ...
    referenceData() {
      if (this.referenceData) {
        // 把要拿的信息重新定义
        let fillData = {
          status: business.shipmentOrderStatus.wait,
        ...
        for (let i = 0; i < oldOrderItems.length; i++) {
          ...
          discount_rate = oldOrderItems[i].discount_rate;
          discount_rate2 = oldOrderItems[i].discount_rate2;
          if (this.referenceData.is_return === 0) {
            quantity = oldOrderItems[i].remain_shipment_quantity;
          } else if (this.referenceData.is_return === 1) {
            quantity = oldOrderItems[i].remain_shipment_return_quantity;
          }
          ...
          orderItems.push(Object.assign({
            sale_order_item_id: oldOrderItems[i].id,
            ...
            quantity: quantity,
            quotation: oldOrderItems[i].quotation,
            unit_price: oldOrderItems[i].unit_price,
    ...
    "form.customer_id"(newVal) {
      ...
      this.$http.get('/customerarchive/info', {params: params}).then(res => {
        if (res.data.code === 0) {
          let defaultAddress = res.data.data.default_address;
          let address;
          if (this.commonApi.isEmpty(defaultAddress)) {
            address = '';
          } else {
            address = defaultAddress.address;
          }
          this.$set(this.form, 'shipment_address', address);
        } else {
  ...
  methods: {
    ...
    // 打开到货编辑框
    openWarehouseOutEdit(row, isReturn = 0) {
      this.currentWarehouseOut = Object.assign({}, row);
      this.currentWarehouseOut.is_return = isReturn;
    ...
    fillFormByRemote(order_id) {
      ...
      this.$http.get("/shipmentorder/info", {params: params}).then(res => {
        if (res.data.code === 0) {
    ...
    addOrderItem() {
      ...
      if (this.form.status == business.shipmentOrderStatus.changing) {
    ...
    cancelOpen(id) {
      ...
      this.$http.post('/shipmentorder/cancelOpen', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
    ...
    audit(id) {
      const loading = this.$loading({lock: true});
      this.$http.post('/shipmentorder/audit', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.fillFormByRemote(id);
    ...
    restoreAudit(id) {
      const loading = this.$loading({lock: true});
      this.$http.post('/shipmentorder/restoreAudit', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.fillFormByRemote(id);
    ...
    change(id) {
      const loading = this.$loading({lock: true});
      this.$http.post('/shipmentorder/change', {id: id}).then(res => {
        loading.close();
        if (res.data.code === 0) {
          this.$message.success(res.data.msg);
          this.fillFormByRemote(id);
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          if (this.form.order_items && this.form.order_items.length > 0) {
            this.loading = true;
            this.$http.post('/shipmentorder/edit', this.form).then(res => {
              this.loading = false;
              if (res.data.code === 0) {
